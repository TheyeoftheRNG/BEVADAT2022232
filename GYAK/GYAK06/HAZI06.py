...
for y in range(1,100,10):
    for x in range (1,100,10):
        try:        
            classifier = DecisionTreeClassifier( x,y)
            classifier.fit(X_train, Y_train)

            Y_pred = classifier.predict(X_test)
            print(accuracy_score(Y_test, Y_pred) , "min_samples_split:", x ,"max_depth:", y)
        except:
            print(x , y ,"error")
    
ezekel a for loopal kerestem nagyából hogy mere van az optmiális és ha lefutott
akkor a legjobb értéknek a helye körülre álítom a for loopot és ott 1-esével nézi végig

try catch-el meg az errorokat skipelem hogy forloopal meg ne áljon le,
mert ha túl nagy a külömbség a max_depth és a min_sample_split között akkot error-t dob
'''