...
class KMeansOnDigits:
    ...
    def get_labels(self):
        result = np.zeros_like(self.n_clusters)
        for cluster in np.unique(self.n_clusters):
    ...
    def calc_accuracy(self) -> float:
        self.calc_accuracy = round(metrics.accuracy_score(self.digits,self.labels), 2)
    
    def plot_confusion_matrix(self):
        cm = confusion_matrix(self.digits.target, self.labels)
